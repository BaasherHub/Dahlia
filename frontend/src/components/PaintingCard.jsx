...
export default function PaintingCard({ painting }) {
  ...
  const toggleWishlist = (e) => {
    ...
    const wishlist = JSON.parse(localStorage.getItem('wishlist') || '[]');
    if (isInWishlist) {
      localStorage.setItem('wishlist', JSON.stringify(wishlist.filter(item => item.id !== painting.id)));
  ...
  return (
    <Link to={`/paintings/${painting.id}`} className="painting-card"
      ...
      <div className="painting-card__img-wrap">
        {imageUrl ? (
          <img src={imageUrl} alt={painting.title || 'Artwork'}
        ...
        <button className={`painting-card__wishlist ${isInWishlist ? 'active' : ''}`}
          onClick={toggleWishlist} aria-label={isInWishlist ? 'Remove from wishlist' : 'Add to wishlist'}>
        ...
        <div className="painting-card__hover"><span>View Details</span></div>
      </div>
      <div className="painting-card__info">
        ...
        {displayPrice && <div className="painting-card__price">{formatPrice(displayPrice)}</div>}
      </div>
    </Link>
  );
}
